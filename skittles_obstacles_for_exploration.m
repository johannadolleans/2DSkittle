...
%% Information regarding the Session
    ...
    ObsHit=[];
    if block >1
        Calling=strcat(subject,'_',num2str(visit),'_',num2str(session),'_',num2str(block-1),'_release');
        load(Calling)
        Score=releaseData(end,end);
        
        if session >1
        Calling=strcat(subject,'_',num2str(visit),'_',num2str(session-1),'_',num2str(block-1),'_release');
        load(Calling)
        Score=releaseData(end,end);  
        end
    else
        if session >1
        Calling=strcat(subject,'_',num2str(visit),'_',num2str(session-1),'_',num2str(block),'_release');
        load(Calling)
        Score=releaseData(end,end);  
        end
    end
   ...
   target=[xtarget, ytarget];
    ...
    f=0;
    if block > 1
        Calling=strcat(subject,'_',num2str(visit),'_',num2str(session),'_',num2str(block-1),'_','Obstacles');
        load(Calling);
%         Obstacles1=Obstaclex;
 f=size(Obstacles1,1);
    end
    if session > 1
            Calling=strcat(subject,'_',num2str(visit),'_',num2str(session-1),'_',num2str(block+1),'_','Obstacles');
        load(Calling);
%         Obstacles1=Obstaclex;
 f=size(Obstacles1,1);
    end
    ...
    fid = fopen(sprintf('%s_%d_%d_%d.bin', subject,visit, session, block),'w');
    ...
    releaseData=zeros(trialsPerBlock,5);
    Obsx=0;
    Obsy=0;
    Obstaclex=[];
    k=1; 
...
%        set(ms1, 'position', [1100 75 50 50]); %makes box bigger
       ...
       ms1 = msgbox(sprintf( 'Nice try, keep up the good work!!'));     %create msgbox
th = findall(ms1, 'Type', 'Text');                   %get handle to text within msgbox
th.FontSize = 20; 
     set(ms1, 'position', [1060 350 350 75]); %makes box bigger
                end
...
ObsHit(f)=0
   ...
   %% end of acquisition and saving of data
    delete(lh);
    fclose(fid);
    save(sprintf('%s_%d_%d_%d_releases.mat',subject,visit, session, block), 'releaseData');
    save(sprintf('%s_%d_%d_%d_Obstacles.mat', subject,visit, session, block), 'Obstacles1','ObsHit');
    close all;
